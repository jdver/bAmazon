   ...
   */
        inquirer
            .prompt([
                {
                    ...
                    choices: function () {
                        var choiceArray = []
                        for (var i = 0; i < res.length; i++) {
                            choiceArray.push(res[i].sku, res[i].product_name, res[i].department_name, res[i].price, res[i].stock_quantity)
                        }
                        return choiceArray
                    },
                    message: 'What is the product sku number?',
            ...
            .then(function(input) {
                ...
                connection.query(queryStr, {sku: item}, function(err, res) {
                    if (err) throw err;
                    ...
                    if (res.length === 0) {
                        console.log('ERROR: Invalid sku. Please select a valid Item ID.');
                        displayInventory();
...
/*
// MOST RECENT COMMENTED OUT 9:52PM
            .then(function (answer) {
                // get the information of the chosen item
                var chosenItem
                for (var i = 0; i < res.length; i++) {
                    if (res[i].sku === answer.choice) {
                        chosenItem = res[i]
                    }
                }
